...
4. Compteur de sauvegardes/clonages — attendait "les règles de séparation Mode Intime effectivement implémentées en base" : **la fondation existe désormais** (`intimate_personas`, voir plus haut), mais ce point précis reste un chantier à part entière, pas encore commencé — compteur lui-même (colonne + incrémentation), et décision produit sur comment/si il s'affiche sous la persona intime plutôt que le pseudo réel.
