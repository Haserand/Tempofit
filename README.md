...
**Chantier en cours : "Pulses/Leaderboard" — fondations SQL (02/08) + première UI "Ma persona intime" (`SettingsView.jsx`, 02/08) branchées, en attente de confirmation par un vrai build Vercel.** Toujours aucun pulse, aucun leaderboard — seulement un bouton qui révèle son propre pseudonyme stable. Prochain chantier logique une fois confirmé : soit continuer ce chantier (opt-in au leaderboard, affichage du classement lui-même), soit retour à l'ordre de priorité normal (Vague 2, Chantier 2 — renforcement post-hoc, moteur BPM/structuration + analyse post-séance, `useSessionAnalysis.js`).
...
- ⚠️ **`get_or_create_intimate_persona()` elle-même reste non testée en conditions réelles** — dépend de `auth.uid()`, qui vaut `null` depuis l'éditeur SQL (pas de session utilisateur réelle là-bas). Une UI l'appelle désormais pour de vrai (voir juste en dessous), donc vérifiable dès qu'un compte authentifié clique le bouton — tant que ce n'est pas confirmé par le prochain build/test manuel, considérer que l'insert/l'idempotence/le `security definer` restent non validés en pratique (seul `generate_intimate_pseudonym`, appelée en isolation, l'a été).
...
4. Compteur de sauvegardes/clonages — seulement une fois les règles de séparation Mode Intime ci-dessus effectivement implémentées en base.
