...
**Chantier en cours : "Pulses/Leaderboard" — fondations SQL de la persona intime posées le 02/08, RIEN DE PLUS.** Aucune UI ne les utilise encore, aucun pulse, aucun leaderboard. Prochain chantier logique : brancher un premier écran dessus (voir plus bas) — sinon, retour à l'ordre de priorité normal (Vague 2, Chantier 2 — renforcement post-hoc, moteur BPM/structuration + analyse post-séance, `useSessionAnalysis.js`).
...
- ⚠️ **PAS ENCORE testé** : `get_or_create_intimate_persona()` elle-même — dépend de `auth.uid()`, qui vaut `null` depuis l'éditeur SQL (pas de session utilisateur réelle là-bas). Ne sera vérifiable qu'une fois une vraie session authentifiée l'appelle, typiquement depuis l'app une fois qu'une UI l'utilisera. Tant que ce point n'est pas confirmé, considérer que seule LA MOITIÉ de ce chantier (le générateur de pseudonyme) est réellement validée en conditions réelles — la RPC elle-même (l'insert, l'idempotence, le `security definer`) reste non testée.
