        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="flex justify-between bg-gray-50 dark:bg-gray-900 p-2 rounded-xl border border-divider">
            {AVAILABLE_ICONS.slice(0, isNaughtyMode ? 14 : 8).map(icon => (
              <button key={icon} onClick={() => setNewRoutineIcon(icon)} className={"text-xl p-2 rounded-lg transition-transform " + (newRoutineIcon === icon ? 'bg-white dark:bg-gray-800 shadow-xs scale-110' : 'grayscale opacity-50')}>{icon}</button>
            ))}
          </div>
          ...
          <div className="pt-2">
            ...
            <select value={newRoutineFreq} onChange={e => setNewRoutineFreq(e.target.value)} className={"w-full rounded-xl px-4 py-3 text-sm font-bold focus:outline-hidden focus:border-blue-500 border " + inputBg + " " + inputBorder + " " + textHighlight}>
