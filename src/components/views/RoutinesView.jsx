...
/**
 * RoutinesView — CORPS SEUL de l'onglet "Routines" (configurations
 * sauvegardées, relançables en un clic). N'inclut PLUS son propre
 * `<ViewHeader/>` ni son propre wrapper `VIEW_CONTENT_WRAPPER` — voir la
 * FUSION du 20/08 ci-dessous.
 *
 * ⚠️ FUSIONNÉ dans PlaylistsView.jsx (20/08, retour direct — "prends du
 ...
 * rend plus que la grille de cartes ; `<ViewHeader/>`/le sélecteur d'onglet
 * vivent maintenant UNE SEULE FOIS dans PlaylistsView.jsx (le titre/
 * sous-titre y changent selon l'onglet actif), pas dupliqués ici.
 *
 * Extrait à l'origine de App.jsx (bloc `view === 'routines'`, 25/07),
 * retiré de la liste des vues de premier niveau le 20/08 (plus de
 * `view === 'routines'` dans App.jsx — voir PlaylistsView.jsx et son prop
 * `initialTab`, même mécanisme que `SettingsView.jsx`). Les routines sont
 * triées côté composant (par nombre de générations manuelles décroissant),
 * mais la mutation de l'état (`setRoutines`, `setRoutineBatchCounts`) reste
 * pilotée depuis App.jsx via les props — ce composant ne fait qu'appeler
 * les setters qu'on lui passe.
 */
