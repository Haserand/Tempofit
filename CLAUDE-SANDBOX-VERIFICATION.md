...
## 4nonies. Reproduction locale d'un bug de build qui échoue à répétition — vérifier le VRAI contenu du dépôt avant de continuer à deviner
Trouvé le 05/08 (incident réel, tests d'un fichier entier échouant avec une erreur React générique "Element type is invalid... undefined") : plusieurs heures passées à tenter de reproduire l'échec localement (import direct du module, stubs `lucide-react`/`supabase-js` fabriqués à la main, tentative de forcer le runtime JSX automatique) — sans jamais reproduire le vrai problème, parce que CHAQUE tentative de reproduction locale a elle-même introduit ses propres artefacts trompeurs (un stub cassé donnant une fausse erreur de syntaxe, `tsx` en ligne de commande utilisant par défaut un runtime JSX différent de celui du vrai projet). La cause réelle — `src/contexts/PlaylistDetailContext.jsx` (le composant) écrasé par erreur avec le contenu de son propre fichier de test, les deux portant presque le même nom — n'a été trouvée qu'en cherchant directement dans le VRAI dépôt GitHub (recherche de code), pas en creusant plus longtemps la reproduction locale.
Règle : si une reproduction locale d'un échec de build échoue plusieurs fois de suite avec des symptômes qui CHANGENT d'une tentative à l'autre (pas le même message d'erreur à chaque essai), considérer que la reproduction locale elle-même a divergé de l'environnement réel (dépendances absentes remplacées par des stubs approximatifs, config JSX/outillage différente) — le signal à suivre. Dans ce cas, arrêter d'empiler des tentatives de reproduction et chercher directement le VRAI contenu du fichier en question (recherche de code GitHub si le dépôt est accessible, ou demander à l'utilisateur de coller le contenu réel) plutôt que de continuer à raffiner un environnement de reproduction qui n'a jamais été fiable dès le départ.
