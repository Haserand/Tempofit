...
### Bloc 23 — 01/09 (suite, audit sémantique des icônes de trophées + 2 principes généralisés)
- `historique/bloc-23.md` — retour direct avec capture ("un chiffre '3' pour Premier Pas fait-il sens ? regarde bien pour tout") : audit complet des 40 icônes — 1 vrai problème structurel trouvé (la famille sessions totales commençait directement par une médaille BRONZE pour la 1ère séance, vidant le mot de son sens) et 1 icône qui suggérait le contraire de son trophée (HIIT avec un graphique qui MONTE, alors qu'un HIIT oscille). Corrigés : 👣🥉🥈🏆 (1/5/30/100 séances, décalé d'un cran), 💓 (HIIT, au lieu de 📈), 🏃 (Marathonien, au lieu d'une médaille générique). 2 principes généralisés dans `readme/partie-02b.md` : justesse sémantique littérale d'une icône (pas seulement unicité/escalade), et garde-fou automatique de longueur de description (48 caractères, filet approximatif — jsdom ne mesure pas de vrai rendu, seule une mesure Playwright réelle confirme)
